...
```toon
components[23]{name,type,path,responsibility}:
  ...
  racing-state,state,src/states/racing-state.ts,main gameplay — dual boats + powerup pipeline + camera + event logging
  ...
  powerup-debug,ui,src/powerup-debug.ts,collapsible POWERUPS panel (spawn + per-powerup tabs) + GENERAL panel (flood/log/live stats)
...
3. `RacingState.update()` runs physics → collision → powerup pipeline → cleanup for both boats independently
...
6. Render phase: clear canvas → `updateCamera(w, h, dt)` → `applyCameraTransform()` → background → zones → pickups → obstacles → both boats → effect visuals → restore → HUD → effects HUD → event log
...
- **Data-driven powerups**: `PowerupDefinition` registry with `onApply/onTick/onExpire` lifecycle hooks — adding a powerup = one definition file
