...
3. `RacingState.update()` runs `updatePhysics()` + `resolveCollisions()` for each boat, then particle emitters + `updateParticles()`
...
5. Render phase: `RacingState.render()` clears canvas → `updateCamera()` → `applyCameraTransform()` → background → particles → both boats → restore → HUD
