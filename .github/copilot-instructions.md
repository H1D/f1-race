...
| collision | `src/systems/collision.ts` | Polygon boundary enforcement + edge-normal wall response with sliding |
...
2. RacingState runs `updatePhysics()` + `resolveMapCollisions()` + particle emitters per boat, then powerup pipeline → cleanup
...
5. Collision: keep boat inside outer polygon + outside island. Edge-normal push + velocity cancellation + tangential sliding
